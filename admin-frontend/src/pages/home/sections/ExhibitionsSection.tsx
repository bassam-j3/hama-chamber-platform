          ...
          {exhibitions.slice(0, 3).map((item) => (
            <Col xs={12} sm={6} lg={4} key={item.id}>
              <Card className="h-100 border-0 rounded-4 overflow-hidden shadow-sm hover-translate-y transition-all bg-white d-flex flex-column">
                 <div className="bg-light d-flex align-items-center justify-content-center w-100" style={{ aspectRatio: '16/9' }}>
                  {item.imageUrl ? (
                ...
                <Card.Body className="p-4 d-flex flex-column flex-grow-1">
                  ...
                  <Card.Text className="text-secondary small mb-0 line-clamp-3 mt-auto">{stripHtml(item.content)}</Card.Text>
                </Card.Body>
              </Card>
            </Col>
          ))}
