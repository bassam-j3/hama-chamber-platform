...
export default function OpportunitiesManagement() {
  ...
  const fetchData = async () => {
    ...
    try { 
      ...
      toast.error('فشل في جلب الفرص الاستثمارية');
    } 
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      toast.success('تم حذف الفرصة بنجاح', { id: toastId });
    } catch { 
  ...
  return (
    <Container fluid className="max-w-75">
            ...
            <Card.Body className="p-4">
              <div className="d-flex justify-content-between align-items-center border-bottom pb-3 mb-4">
                <div className="d-flex align-items-center gap-2">
                  <span className="material-symbols-outlined text-primary fs-2">lightbulb</span>
                  <h4 className="text-primary fw-bold mb-0">الفرص الاستثمارية</h4>
                </div>
                <Button variant="primary" size="sm" onClick={() => navigate('/admin/opportunities/create')} className="fw-bold px-3 d-flex align-items-center gap-1 shadow-sm rounded-pill py-2">
              ...
              {!isLoading && !error && data.length === 0 && (
                <div className="text-center py-5">
                  ...
                  <h4 className="text-muted fw-bold mb-2">لا يوجد فرص استثمارية حالياً</h4>
                  <p className="text-muted mb-4">انقر على زر الإضافة لنشر فرصة استثمارية جديدة للتجار والمستثمرين.</p>
                  <Button variant="primary" onClick={() => navigate('/admin/opportunities/create')} className="px-4 fw-bold rounded-pill">
                    إضافة فرصة جديدة
                  </Button>
                </div>
              )}
                    ...
                    {isLoading ? (
                      ...
                      data.map((item) => (
                        <tr key={item.id}>
                          ...
                          <td>
                            <Button variant="outline-primary" size="sm" className="me-2 fw-bold" onClick={() => navigate(`/admin/opportunities/edit/${item.id}`, { state: { opportunityItem: item } })}>
      ...
      <Modal show={showDeleteModal} onHide={() => setShowDeleteModal(false)} centered backdrop="static">
        ...
        <Modal.Body className="fs-6 text-secondary pt-3 pb-4 px-4">هل أنت متأكد من رغبتك في حذف هذه الفرصة الاستثمارية نهائياً؟</Modal.Body>
