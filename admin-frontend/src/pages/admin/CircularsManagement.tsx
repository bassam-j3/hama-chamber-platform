  ...
  return (
    <Container fluid className="max-w-75">
          ...
          <Card className="shadow-sm border-0 rounded-4">
            <Card.Body className="p-4">
              <div className="d-flex justify-content-between align-items-center border-bottom pb-3 mb-4">
                ...
                <Button variant="primary" size="sm" onClick={() => navigate('/admin/circulars/create')} className="fw-bold px-3 d-flex align-items-center gap-1 shadow-sm rounded-pill py-2">
              ...
              {!isLoading && !error && circulars.length === 0 && (
                <div className="text-center py-5">
                  ...
                  <h4 className="text-muted fw-bold mb-2">لا يوجد تعاميم مضافة حالياً</h4>
                  <p className="text-muted mb-4">انقر على زر الإضافة لنشر تعميم أو قرار جديد.</p>
                  <Button variant="primary" onClick={() => navigate('/admin/circulars/create')} className="px-4 fw-bold rounded-pill">
                    إضافة تعميم جديد
                  </Button>
                </div>
              )}
              ...
              {(isLoading || circulars.length > 0) && !error && (
                <Table responsive hover className="align-middle text-center border-top">
                  <thead className="table-light">
                    <tr>
                      <th className="text-secondary font-monospace text-start">عنوان التعميم</th>
                      <th className="text-secondary font-monospace">التصنيف</th>
                    ...
                    {isLoading ? (
                      ...
                      circulars.map((item) => (
                        <tr key={item.id}>
                          ...
                          <td><Badge bg="info" text="dark" className="px-3 py-2 rounded-pill shadow-sm fw-normal">{item.category}</Badge></td>
                          ...
                          <td>
                            <Button variant="outline-primary" size="sm" className="me-2 fw-bold" onClick={() => navigate(`/admin/circulars/edit/${item.id}`, { state: { circularItem: item } })}>
      ...
      <Modal show={showDeleteModal} onHide={() => setShowDeleteModal(false)} centered backdrop="static">
        ...
        <Modal.Body className="fs-6 text-secondary pt-3 pb-4 px-4">هل أنت متأكد من رغبتك في حذف هذا التعميم نهائياً؟</Modal.Body>
