...
export default function LawForm() {
  ...
  const populateForm = useCallback((data: Law) => {
  ...
  useEffect(() => {
    if (id) {
      const stateItem = location.state?.item as Law | undefined;
      if (stateItem) {
        populateForm(stateItem);
      } else {
        setIsLoading(true);
        axiosInstance.get("/laws").then(res => {
          const item = res.data.find((p: Law) => p.id === id);
          if (item) populateForm(item);
        }).catch(() => {
          toast.error("فشل في تحميل بيانات القانون");
      ...
      <Card className="shadow-sm border-0 rounded-4">
        <Card.Body className="p-5">
          ...
          <Form onSubmit={handleSubmit(onSubmit)}>
            ...
            <Form.Check type="switch" label="نشر القانون" {...register("isActive")} className="mb-4 fw-bold" id="isActive" />
