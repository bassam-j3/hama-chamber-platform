  ...
  return (
    <Container fluid className="max-w-75">
            ...
            <Card.Body className="p-4">
              <div className="d-flex justify-content-between align-items-center border-bottom pb-3 mb-4">
                <div className="d-flex align-items-center gap-2">
                  <span className="material-symbols-outlined text-primary fs-2">storefront</span>
                  <h4 className="text-primary fw-bold mb-0">المعارض والمؤتمرات</h4>
                </div>
                <Button variant="primary" size="sm" onClick={() => navigate('/admin/exhibitions/create')} className="fw-bold px-3 d-flex align-items-center gap-1 shadow-sm rounded-pill py-2">
                  <span className="material-symbols-outlined fs-6">add</span> إضافة معرض جديد
                </Button>
              </div>
              ...
              {!isLoading && !error && data.length === 0 && (
                <div className="text-center py-5">
                  ...
                  <h4 className="text-muted fw-bold mb-2">لا يوجد معارض أو مؤتمرات حالياً</h4>
                  <p className="text-muted mb-4">انقر على زر الإضافة لتوثيق مشاركات الغرفة في المعارض المحلية والدولية.</p>
                  <Button variant="primary" onClick={() => navigate('/admin/exhibitions/create')} className="px-4 fw-bold rounded-pill">
                    إضافة معرض جديد
                  </Button>
                </div>
              )}
              ...
              {(isLoading || data.length > 0) && !error && (
                <Table responsive hover className="align-middle text-center border-top">
                  <thead className="table-light">
                    <tr>
                      <th className="text-secondary font-monospace text-start">اسم المعرض</th>
                    ...
                    {isLoading ? (
                      ...
                      data.map((item) => (
                        <tr key={item.id}>
                          ...
                          <td>
                            <Button variant="outline-primary" size="sm" className="me-2 fw-bold" onClick={() => navigate(`/admin/exhibitions/edit/${item.id}`, { state: { exhibitionItem: item } })}>
      ...
      <Modal show={showDeleteModal} onHide={() => setShowDeleteModal(false)} centered backdrop="static">
        ...
        <Modal.Body className="fs-6 text-secondary pt-3 pb-4 px-4">هل أنت متأكد من رغبتك في حذف هذا المعرض نهائياً؟</Modal.Body>
