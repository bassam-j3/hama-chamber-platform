...
export default function NewsManagement() {
  ...
  const fetchData = async () => {
    ...
    try { const response = await axiosInstance.get("/news"); setNewsList(response.data); } 
    ...
    <Container fluid className="max-w-75">
      <Row className="justify-content-center"><Col lg={12}><Card className="shadow-sm border-0 rounded-4"><Card.Body className="p-4">
        <div className="d-flex justify-content-between align-items-center border-bottom pb-3 mb-4">
          <div className="d-flex align-items-center gap-2"><span className="material-symbols-outlined text-primary fs-2">newspaper</span><h4 className="text-primary fw-bold mb-0">الأخبار والنشاطات</h4></div>
          <Button variant="primary" size="sm" onClick={() => navigate('/admin/news/create')} className="fw-bold px-3 d-flex align-items-center gap-1 shadow-sm"><span className="material-symbols-outlined fs-6">add</span> إضافة خبر جديد</Button>
        </div>
        ...
        ) : newsList.length === 0 ? ( <div className="text-center p-5 text-muted"><span className="material-symbols-outlined fs-1 mb-2">inbox</span><h5>لا يوجد أخبار مضافة حالياً</h5></div>
        ) : (
          <Table responsive hover className="align-middle text-center border-top">
            <thead className="table-light"><tr><th className="text-secondary font-monospace">صورة</th><th className="text-secondary font-monospace text-start">عنوان الخبر</th><th className="text-secondary font-monospace">تاريخ النشر</th><th className="text-secondary font-monospace">الحالة</th><th className="text-secondary font-monospace">الإجراءات</th></tr></thead>
            <tbody>
              {newsList.map((item) => (
                <tr key={item.id}>
                  <td><div className="rounded overflow-hidden mx-auto bg-light border d-flex align-items-center justify-content-center" style={{ width: '60px', height: '40px' }}>{item.imageUrl ? <img src={item.imageUrl} alt={item.title} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <span className="material-symbols-outlined text-muted mt-1">image</span>}</div></td>
                  <td className="fw-bold text-dark text-start">{item.title}</td>
                  <td>{new Date(item.createdAt).toLocaleDateString('ar-SY')}</td>
                  <td>{item.isActive ? <Badge bg="success" className="px-3 py-2 rounded-pill shadow-sm">منشور</Badge> : <Badge bg="secondary" className="px-3 py-2 rounded-pill shadow-sm">مسودة</Badge>}</td>
                  <td>
                    <Button variant="outline-primary" size="sm" className="me-2 fw-bold" onClick={() => navigate(`/admin/news/edit/${item.id}`, { state: { newsItem: item } })}>تعديل</Button>
