...
interface Circular { id: string; title: string; category: string; isActive: boolean; createdAt: string; }
...
export default function CircularsManagement() {
  ...
  const fetchData = async () => {
    ...
    try { const response = await axiosInstance.get("/circulars"); setCirculars(response.data); } 
    ...
    <Container fluid className="max-w-75">
      <Row className="justify-content-center"><Col lg={12}><Card className="shadow-sm border-0 rounded-4"><Card.Body className="p-4">
        <div className="d-flex justify-content-between align-items-center border-bottom pb-3 mb-4">
          <div className="d-flex align-items-center gap-2"><span className="material-symbols-outlined text-primary fs-2">assignment</span><h4 className="text-primary fw-bold mb-0">التعاميم والقرارات</h4></div>
          <Button variant="primary" size="sm" onClick={() => navigate('/admin/circulars/create')} className="fw-bold px-3 d-flex align-items-center gap-1 shadow-sm"><span className="material-symbols-outlined fs-6">add</span> إضافة تعميم جديد</Button>
        </div>
        ...
        ) : circulars.length === 0 ? ( <div className="text-center p-5 text-muted"><span className="material-symbols-outlined fs-1 mb-2">inbox</span><h5>لا يوجد تعاميم مضافة حالياً</h5></div>
        ) : (
          <Table responsive hover className="align-middle text-center border-top">
            <thead className="table-light"><tr><th className="text-secondary font-monospace text-start">عنوان التعميم</th><th className="text-secondary font-monospace">التصنيف</th><th className="text-secondary font-monospace">تاريخ النشر</th><th className="text-secondary font-monospace">الحالة</th><th className="text-secondary font-monospace">الإجراءات</th></tr></thead>
            <tbody>
              {circulars.map((item) => (
                <tr key={item.id}>
                  <td className="fw-bold text-dark text-start">{item.title}</td>
                  <td><Badge bg="info" text="dark" className="px-3 py-2 rounded-pill shadow-sm">{item.category}</Badge></td>
                  ...
                  <td>
                    <Button variant="outline-primary" size="sm" className="me-2 fw-bold" onClick={() => navigate(`/admin/circulars/edit/${item.id}`, { state: { circularItem: item } })}>تعديل</Button>
